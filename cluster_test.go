...
var slotsMultiRespWithMultiReplicas = NewResult(slicemsg('*', []ValkeyMessage{
	slicemsg('*', []ValkeyMessage{
		...
		slicemsg('*', []ValkeyMessage{ // replica1
			strmsg('+', "127.0.0.2"),
			{typ: ':', intlen: 1},
			strmsg('+', ""),
		}),
		slicemsg('*', []ValkeyMessage{ // replica2
			strmsg('+', "127.0.0.3"),
			{typ: ':', intlen: 2},
			strmsg('+', ""),
		}),
		slicemsg('*', []ValkeyMessage{ // replica3
			strmsg('+', "127.0.0.4"),
			{typ: ':', intlen: 3},
			strmsg('+', ""),
		}),
...
var shardsResp = NewResult(slicemsg(typeArray, []ValkeyMessage{
	slicemsg(typeMap, []ValkeyMessage{
		...
		slicemsg(typeArray, []ValkeyMessage{
			slicemsg(typeMap, []ValkeyMessage{ // failed master
			...
			slicemsg(typeMap, []ValkeyMessage{ // master
				strmsg(typeBlobString, "id"),
				strmsg(typeBlobString, ""),
			...
			slicemsg(typeMap, []ValkeyMessage{ // replica
				strmsg(typeBlobString, "id"),
				strmsg(typeBlobString, ""),
...
var shardsRespTls = NewResult(slicemsg(typeArray, []ValkeyMessage{
	slicemsg(typeMap, []ValkeyMessage{
		...
		strmsg(typeBlobString, "nodes"),
		slicemsg(typeArray, []ValkeyMessage{
			slicemsg(typeMap, []ValkeyMessage{ // replica, tls
				strmsg(typeBlobString, "id"),
				strmsg(typeBlobString, ""),
			...
			slicemsg(typeMap, []ValkeyMessage{ // failed master, tls + port
				strmsg(typeBlobString, "id"),
				strmsg(typeBlobString, ""),
			...
			slicemsg(typeMap, []ValkeyMessage{ // master, tls + port
				strmsg(typeBlobString, "id"),
				strmsg(typeBlobString, ""),
			...
			slicemsg(typeMap, []ValkeyMessage{ // replica, port
				strmsg(typeBlobString, "id"),
				strmsg(typeBlobString, ""),
...
func TestClusterClientCacheASKRetry(t *testing.T) {
	...
	t.Run("DoCache Retry on ASK with StaticClientTTL", func(t *testing.T) {
		...
		m.DoMultiFn = func(multi ...Completed) *valkeyresults {
			askDone = true
			// askingMultiCache under .ToStaticTTL() must emit a stride-3
	...
	t.Run("DoCache Retry on ASK with StaticClientTTL transport error", func(t *testing.T) {
		...
		m.DoMultiFn = func(multi ...Completed) *valkeyresults {
			...
			if !errInjected {
				...
				// cmd offset (stride-3, offset 2) to exercise the .ToStaticTTL()
	...
	t.Run("DoMultiCache Retry on ASK with mixed StaticClientTTL strips tag", func(t *testing.T) {
		...
		_ = client.DoMultiCache(context.Background(),
			CT(client.B().Get().Key("k1").Cache().ToStaticTTL(), 10*time.Second),
